...
private:
    ...
    ros::Publisher pubglobalMapKeyPosesDS;
...
public:
        ...
        nh("~")
    {
        ...
        pubglobalMapKeyPosesDS = nh.advertise<sensor_msgs::PointCloud2>("/1", 2);
...
//三、下采样
    void downsampleCurrentScan(){
        ...
        downSizeFilterCorner.setInputCloud(laserCloudCornerLast);//加入点云
        ...
        *laserCloudSurfTotalLast += *laserCloudOutlierLastDS;
        ...
        //得到下采样点云laserCloudCornerLastDS   laserCloudSurfLastDS   laserCloudOutlierLastDS   laserCloudSurfTotalLastDS
...
//*****************************************************************************************************************************************************
    ...
    void publishKeyPosesAndFrames(){
        ...
        if (pubKeyPoses.getNumSubscribers() != 0){
            ...
            if (cloudKeyPoses3D->points.size()==300)
            {
                for(int i=0;i<cloudKeyPoses3D->points.size();i++)
                {
                    cout<<cloudKeyPoses3D->points[i].x<<" "<<cloudKeyPoses3D->points[i].y<<" "<<cloudKeyPoses3D->points[i].z<<endl;
                }
            }
...
//线程可视化
    ...
    void publishGlobalMap(){
        ...
        downSizeFilterGlobalMapKeyFrames.setInputCloud(globalMapKeyFrames);  //无颜色
        ...
        for(int i=0;i<globalMapKeyFramesDS->points.size();i++)
        {
            ...
            point.x = globalMapKeyFramesDS->points[i].x;                                                        
            point.y = globalMapKeyFramesDS->points[i].y;                                                        
            point.z = globalMapKeyFramesDS->points[i].z;
            point.r = globalMapKeyFramesDS->points[i].r;
            point.g = globalMapKeyFramesDS->points[i].g;
            point.b = globalMapKeyFramesDS->points[i].b;
            colored_cloud->points.push_back (point);  
        }
        //******************************test*************************************
        sensor_msgs::PointCloud2 cloudMsg1;
        pcl::toROSMsg(*colored_cloud, cloudMsg1);
        cloudMsg1.header.frame_id = "/velodyne";
        pubglobalMapKeyPosesDS.publish(cloudMsg1);
        //******************************test*************************************
...
//--------------------------------------------------------------------------------------------------------------------------------------
    ...
    pcl::PointCloud<PointType>::Ptr transformPointCloud(pcl::PointCloud<PointType>::Ptr cloudIn){
        ...
        for (int i = 0; i < cloudSize; ++i){
            ...
            pointTo.r = cloudIn->points[i].r;
            pointTo.g = cloudIn->points[i].g;
            pointTo.b = cloudIn->points[i].b;
